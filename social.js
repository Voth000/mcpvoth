...
gameStateText["start"] = "Internet Star - Going viral puts you in the spotlight, but keeping up the hype can be a real struggle.";
...
function handleButtonClick(command) {
    ...
    if (command === '101') {
        setTimeout(normal, 2000); // Show the "again" button after 4 seconds
    };
    if (command === 'book') {
...
document.querySelector('button[data-command="drama"]').addEventListener('click', function () {
    hideContentDiv();
    con();
...
function handleCommand(command) {
    if (command === '101') {
        return {
            message: "Hey, you used to be the go-to for online drama, huh? Well, it seems like the tide has turned, and folks are just over the constant extravagant and nonsensical drama. It's bringing in some negative vibes, babe. Time for a change, maybe?",
        };
    } else if (command === 'options') {
        return {
            message: "You died while filming youtube video!!!",
            
        };
    } else if (command === 'silver') {
        return {
            message: "You got into acting or modeling through your parents' connections, and that's the end of it. You should explore other things to do.",
        };
